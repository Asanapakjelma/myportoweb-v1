...
import { SiExpress, SiMui, SiMysql, SiTailwindcss } from 'react-icons/si';
				...
				{techs.map((tech, index) => (
					<div key={index}>
						...
						<div className='tech-row'>
							{tech.items.map((items, index) => (
								<GlowBox key={index} icon={items.icon} color={items.color} title={items.title} />
							))}
						</div>
					</div>
				))}
